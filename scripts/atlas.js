...
const getStatusDetails = (status) => {
  ...
  // ⭐️ CRITICAL FIX: Standardize the status string for accurate matching
  // 1. Remove EMOJI/special characters (which break overall status matching)
  // 2. Convert to UPPERCASE (Fixes the "Red"/"Amber" vs "RED"/"AMBER" case mismatch)
  // 3. Trim whitespace (Handles leading/trailing spaces)
  const standardizedStatus = status
    .replace(
      /(\u00a9|\u00ae|[\u2000-\u3300]|\ud83c[\ud000-\udfff]|\ud83d[\ud000-\udfff]|\ud83e[\ud000-\udfff])/g,
      ""
    )
  ...
  // Use the fully standardized status string for the switch
  switch (standardizedStatus) {
    ...
    // These now correctly match "Red", "Amber", etc., after conversion to UPPERCASE.
    case "RED":
      return {
        color: "text-red-900",
        icon: "🔴",
...
function renderOverallStatus(overall) {
  ...
  if (card) {
    ...
    const cleanStatus = overall.status
      .replace(
        /(\u00a9|\u00ae|[\u2000-\u3300]|\ud83c[\ud000-\udfff]|\ud83d[\ud000-\udfff]|\ud83e[\ud000-\udfff])/g,
        ""
      )
  ...
  document.getElementById(
    "lastUpdated"
  ).textContent = `Last Update: ${overall.date}`;
...
function renderIndicatorTable(tableId, indicators) {
  ...
  indicators.forEach((indicator) => {
    ...
    const indicatorValue =
      indicator.value === 0.0 || indicator.value === null
        ? "N/A"
        : indicator.value;
    ...
    if (Array.isArray(indicatorValue) && indicatorValue.length === 2) {
      ...
      if (indicator.id === "SPX_INDEX" || indicator.id === "ASX_200") {
        displayValue = indicatorValue.toLocaleString(undefined, {
    ...
    if (currentStatus === "N/A" || currentStatus === "ERROR") {
      ...
      const sourceURL =
        indicator.source_link || indicator.source || indicator.url;
    ...
    // --- START OF ICON RENDERING FIX (IMPLEMENTED CHANGE) ---
    let iconHtml;
    const iconValue = details.icon;

    if (iconValue === "red" || iconValue === "amber" || iconValue === "green") {
      // This is the requested change: remove the dot by setting iconHtml to an empty string.
      iconHtml = "";
    } else {
      // Keep the default emoji/icon rendering for 4-Tier statuses if they exist
      iconHtml = `<span class="mr-2">${iconValue}</span>`;
    }
    // --- END OF ICON RENDERING FIX (IMPLEMENTED CHANGE) ---
    ...
    row.innerHTML = `
            ...
            <td class="w-24 px-3 py-3 text-sm font-semibold text-gray-700 whitespace-nowrap">
                ${displayValue}
                <span class="px-2 py-0.5 ml-2 text-xs font-bold rounded-full uppercase ${
                  details.badge
                }">
...
function renderNewsFeed(newsArticles) {
  ...
  if (!Array.isArray(newsArticles) || newsArticles.length === 0) {
    newsListContainer.innerHTML =
      '<p class="text-gray-500">No contextual news articles were retrieved for this analysis.</p>';
    return;
  }

  newsListContainer.innerHTML = newsArticles
    .map((article) => {
      ...
      return `
            <div class="border-b border-gray-100 pb-4 last:border-b-0">
                ...
                <p class="text-sm text-gray-600 mt-1">${
                  article.snippet || "Snippet unavailable."
                }</p>
...
function initializeNarrativePage() {
  fetchAtlasData().then((data) => {
    ...
    document.getElementById(
      "narrativeTitle"
    ).textContent = `${cleanStatus} Risk Posture Analysis`;
    document.getElementById(
      "narrativeDate"
    ).textContent = `Updated: ${overall.date}`;
    ...
    const narrativeContainer = document.getElementById(
      "dailyNarrativeContainer"
    );
    const dailyNarrative =
      overall.daily_narrative || "Narrative is currently unavailable.";

    if (narrativeContainer) {
      const narrativeParagraphs = dailyNarrative
        .split("\n")
        .filter((p) => p.trim() !== "")
        .map((p) => `<p class="mb-4">${p.trim()}</p>`)
        .join("");
      narrativeContainer.innerHTML =
        narrativeParagraphs || `<p>Analysis failed to load.</p>`;
    }
...
function renderArchivePosts(startIndex) {
  ...
  const postsToRender = allArchivePosts.slice(
    startIndex,
    startIndex + POSTS_PER_LOAD
  );
  ...
  if (postsToRender.length === 0 && startIndex > 0) {
    ...
    if (container)
      container.innerHTML =
        "<p class='text-gray-500'>No historical narratives found in the archive.</p>";
    return;
  }
  ...
  postsToRender.forEach((post) => {
    ...
    const narrativeHTML = (
      post.daily_narrative || "No detailed narrative provided."
    )
      .split("\n\n")
...
async function initializeArchivePage() {
  ...
  if (!response.ok) {
    ...
    document.getElementById("archiveContainer").innerHTML =
      "<p class='text-red-500'>Error loading archive. Check the console and confirm 'data/atlas-archive.json' exists.</p>";
    return;
  }
