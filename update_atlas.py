...
def run_update_process(atlas_data):
    ...
    # Placeholder for actual narrative generation
    vix_value = next((i["value"] for i in atlas_data["macro"] if i["id"] == "VIX"), 0.0)
    atlas_data["overall"]["daily_narrative"] = (
        f"Today's Atlas analysis, dated {atlas_data['overall']['date']}, shows continued stability across global equities, supported by low implied volatility (VIX at {vix_value:.2f})."
        "\n\nHowever, the recent dip in Gold and Crude prices suggests a soft patch in commodity demand, and the spread between the 3Y and 30Y Treasury yield continues to indicate long-term economic deceleration. "
        "We are actively monitoring the high-yield credit market for further deterioration."
    )

    # 5. Archive the daily narrative (FOR INFINITE SCROLL)
    save_to_archive(atlas_data["overall"])

    # 6. Return the updated data structure
    return atlas_data
